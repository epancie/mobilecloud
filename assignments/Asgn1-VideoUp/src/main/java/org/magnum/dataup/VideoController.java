 ...
 * 
 * Ver https://class.coursera.org/mobilecloud-001/forum/thread?thread_id=545
 * 
 * 
 */
	...
	public @ResponseBody VideoStatus setVideoData( @PathVariable ("id") long id, 
		...
		try {
			...
			if (v!=null){
				...
				response.setStatus(404, "error:Video " + id + " Not found" );
				return new VideoStatus(VideoState.PROCESSING);
			}
		}
		catch(IndexOutOfBoundsException e){
			LOG.output("Exception " + e.toString() );
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST, "error:" + e.toString());
			return new VideoStatus(VideoState.PROCESSING);
	...
	public void getData(@PathVariable ("id") long id, 
		...
		try {
			...
			if (v!=null){			
				...
				response.setStatus(HttpServletResponse.SC_NOT_FOUND, "error:Video " + id + " Not found" );
			}
		}
		catch(IndexOutOfBoundsException e){
